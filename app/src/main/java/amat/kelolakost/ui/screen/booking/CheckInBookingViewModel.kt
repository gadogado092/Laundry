...
class CheckInBookingViewModel(
    ...
    private val _checkInUi: MutableStateFlow<CheckInBookingUi> =
        MutableStateFlow(
            CheckInBookingUi(
                tenantId = "0",
    ...
    val checkInUi: StateFlow<CheckInBookingUi> get() = _checkInUi
    ...
    fun getData(): CheckInBookingUi {
        return checkInUi.value
    }
    ...
    fun getBooking(bookingId: String) {
        ...
        try {
            viewModelScope.launch {
                val data = bookingRepository.getBooking(bookingId)
                _checkInUi.value = checkInUi.value.copy(
                    ...
                    guaranteeCost = data.priceGuarantee.toInt(),
    ...
    fun refreshDataUI() {
        ...
        val nominalBooking = checkInUi.value.nominalBooking

        //HITUNG TOTAL BIAYA
        val total: BigInteger =
            (qty.toBigInteger() * price.toBigInteger()) - nominalBooking.toBigInteger() + extraPrice.toBigInteger() + priceGuarantee.toBigInteger() - discount.toBigInteger()
    ...
    fun dataIsComplete(): Boolean {
        ...
        //checkStatusUnit sebelum check in
        if (checkInUi.value.unitStatusId == "1") {
            _isCheckInSuccess.value =
                ValidationResult(true, "Unit Terisi Penyewa")
            return false
        } else if (checkInUi.value.unitStatusId == "3" || checkInUi.value.unitStatusId == "4") {
            _isCheckInSuccess.value =
                ValidationResult(true, "Unit Tahap Pembersihan atau Perbaikan")
            return false
        }
    ...
    fun processCheckIn() {
        ...
        try {
            viewModelScope.launch {
                ...
                bookingRepository.prosesCheckIn(
                    cashFlow = cashFlow,
...
class CheckInBookingViewModelFactory(
    private val tenantRepository: TenantRepository,
    private val unitRepository: UnitRepository,
    private val bookingRepository: BookingRepository,
    private val userRepository: UserRepository
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(CheckInBookingViewModel::class.java)) {
            return CheckInBookingViewModel(
                tenantRepository,
                unitRepository,
                bookingRepository,
                userRepository
