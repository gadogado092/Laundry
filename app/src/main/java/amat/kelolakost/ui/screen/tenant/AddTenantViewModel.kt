...
class AddTenantViewModel(private val tenantRepository: TenantRepository) : ViewModel() {
    private val _unitTypeUi: MutableStateFlow<TenantUi> =
        MutableStateFlow(TenantUi())
    val unitTypeUi: StateFlow<TenantUi>
        get() = _unitTypeUi
    ...
    fun setNumberPhone(value: String) {
        _isInsertSuccess.value = ValidationResult(true, "")
        _unitTypeUi.value = _unitTypeUi.value.copy(numberPhone = value)
        if (_unitTypeUi.value.numberPhone.trim().isEmpty()) {
            _isNumberPhoneValid.value = ValidationResult(true, "Nomor Harus Terisi")
        } else if (!isNumberPhoneValid(_unitTypeUi.value.numberPhone.trim())) {
    ...
    fun setEmail(value: String) {
        _isInsertSuccess.value = ValidationResult(true, "")
        _unitTypeUi.value = _unitTypeUi.value.copy(email = value)

        if (_unitTypeUi.value.email.trim().isEmpty()) {
            _isEmailValid.value = ValidationResult(true, "Email Wajib Dimasukkan")
        } else if (!isEmailValid(_unitTypeUi.value.email.trim())) {
    ...
    fun setGender(value: Boolean) {
        _unitTypeUi.value = _unitTypeUi.value.copy(gender = value)
    }

    fun setAddress(value: String) {
        _isInsertSuccess.value = ValidationResult(true, "")
        _unitTypeUi.value = _unitTypeUi.value.copy(address = value)
        if (_unitTypeUi.value.address.trim().isEmpty()) {
        ...
        if (_unitTypeUi.value.address.trim().isEmpty()) {
            ...
            viewModelScope.launch {
                ...
                val tenant = Tenant(
                    ...
                    gender = _unitTypeUi.value.gender,
                    numberPhone = _unitTypeUi.value.numberPhone,
                    address = _unitTypeUi.value.address,
                    email = _unitTypeUi.value.email,
                    limitCheckOut = "",
                    unitId = "0",
                    guaranteeCost = 0,
                    additionalCost = 0,
