...
import amat.laundry.ui.component.CashierItem
...
fun CashierScreen(
    ...
    val viewModel: CashierViewModel =
        viewModel(
            factory = CashierViewModelFactory(
    ...
    Column {
        TopAppBar(
            title = {
                Text(
                    text = stringResource(id = R.string.title_cashier),
                    color = FontWhite,
                    fontSize = 22.sp
                )
            },
        ...
        Box(
            ...
            viewModel.stateCashier.collectAsState(initial = UiState.Loading).value.let { uiState ->
                when (uiState) {
                    is UiState.Error -> {
                        ErrorLayout(
